...
def _homogeneity_violation(edata, vm, num_copies, violation_type, verbose=False):
    ...
    if isinstance(edata, Profile):
        ...
        new_edata = Profile(
            list(map(list, rankings)), rcounts=new_rcounts, cmap=edata.cmap
        )
    else:
...
def has_homogeneity_violation(edata, vm, num_copies=2, verbose=False):
    ...
    """
    return _homogeneity_violation(
        edata, vm, num_copies, "Homogeneity", verbose=verbose
    )[0]
...
def has_upward_homogeneity_violation(edata, vm, num_copies=2, verbose=False):
    ...
    """
    return _homogeneity_violation(
        edata, vm, num_copies, "Upward Homogeneity", verbose=verbose
    )[0]
...
def has_downward_homogeneity_violation(edata, vm, num_copies=2, verbose=False):
    ...
    """
    return _homogeneity_violation(
        edata, vm, num_copies, "Downward Homogeneity", verbose=verbose
    )[0]
...
def _has_block_violation(edata, vm, violation_type, verbose=False):
    ...
    if isinstance(edata, Profile):
        ...
        new_edata = Profile(
            list(map(list, old_rankings)) + new_rankings,
            rcounts=list(old_rcounts) + new_rcounts,
            cmap=edata.cmap,
        )
    else:
        old_rankings, old_rcounts = edata.rankings_counts
        ...
        new_rankings = [
            {c: perm.index(c) + 1 for c in edata.candidates}
            for perm in permutations(edata.candidates)
        ]
        new_rcounts = [1] * len(new_rankings)

        new_edata = ProfileWithTies(
            old_rankings + new_rankings,
            rcounts=old_rcounts + new_rcounts,
            candidates=edata.candidates,
            cmap=edata.cmap,
        )
...
def has_upward_block_preservation_violation(edata, vm, verbose=False):
    ...
    """
    return _has_block_violation(
        edata, vm, "Upward Block Preservation", verbose=verbose
    )[0]
...
def has_downward_block_preservation_violation(edata, vm, verbose=False):
    ...
    """
    return _has_block_violation(
        edata, vm, "Downward Block Preservation", verbose=verbose
    )[0]
...
def has_preferential_equality_violation(prof, vm, verbose=False):
    ...
    See Definition 2.1 and Lemma 2.4 from the paper "Characterizations of voting rules based on majority margins"
    by Y. Ding,  W. Holliday, and E. Pacuit.
    """
    ...
    for x, y in combinations(prof.candidates, 2):
        xy_rankings = [
            ...
            if get_rank(r, x) == get_rank(r, y) - 1
            and not any(
                get_rank(r, z) == get_rank(r, x) for z in prof.candidates if z != x
            )
            and not any(
                get_rank(r, z) == get_rank(r, y) for z in prof.candidates if z != y
            )
        ]

        other_rankings_xy = [r for r in prof.rankings if r not in xy_rankings]

        # consider all even-size sub-multisets of xy_rankings
        if len(xy_rankings) >= 2:
            for s in range(2, len(xy_rankings) + 1, 2):
                for subset in combinations(xy_rankings, s):
                    rest_xy = list(xy_rankings)
                    for item in subset:
                        rest_xy.remove(item)

                    for I, J in equal_size_partitions_with_duplicates(list(subset)):
                        new_rankings_I = (
                            [swap_candidates(r, x, y) for r in I]
                            + list(J)
                            + rest_xy
                            + list(other_rankings_xy)
                        )
                        prof_I = prof_constructor(new_rankings_I)
                        new_rankings_J = (
                            [swap_candidates(r, x, y) for r in J]
                            + list(I)
                            + rest_xy
                            + list(other_rankings_xy)
                        )
        ...
        yx_rankings = [
            r
            for r in prof.rankings
            if get_rank(r, y) == get_rank(r, x) - 1
            and not any(
                get_rank(r, z) == get_rank(r, x) for z in prof.candidates if z != x
            )
            and not any(
        ...
        other_rankings_yx = [r for r in prof.rankings if r not in yx_rankings]

        if len(yx_rankings) >= 2:
            for s in range(2, len(yx_rankings) + 1, 2):
                for subset in combinations(yx_rankings, s):
                    rest_yx = list(yx_rankings)
                    for item in subset:
                        rest_yx.remove(item)

                    for I, J in equal_size_partitions_with_duplicates(list(subset)):
                        new_rankings_I = (
                            [swap_candidates(r, y, x) for r in I]
                            + list(J)
                            + rest_yx
                            + list(other_rankings_yx)
                        )
                        prof_I = prof_constructor(new_rankings_I)
                        new_rankings_J = (
                            [swap_candidates(r, y, x) for r in J]
                            + list(I)
                            + rest_yx
                            + list(other_rankings_yx)
                        )
...
def find_all_preferential_equality_violations(prof, vm, verbose=False):
    """
    Return all the preferential equality violations for the voting method vm.
    Returns a list of tuples of three profiles (prof, prof_I, prof_J) such that
    vm(prof_I) != vm(prof_J) and prof_I and prof_J are as defined Lemma 2.4 from
    the paper "Characterizations of voting rules based on majority margins" by
    Y. Ding, W. Holliday, and E. Pacuit (see also Definition 2.1).
    """
    ...
    for x, y in combinations(prof.candidates, 2):
        ...
        # Consider every even-size sub-multiset of xy_rankings
        if len(xy_rankings) >= 2:
            for s in range(2, len(xy_rankings) + 1, 2):
                for subset in combinations(xy_rankings, s):
                    rest_xy = list(xy_rankings)
        ...
        if len(cands) > 1:
            ...
            for r1, r2 in combinations(rankings_with_tie, 2):
                ...
                for lin_order, reverse_lin_order in linear_orders_with_reverse(cands):
                    other_rankings = [
                        r for r in prof.rankings if r not in rankings_with_tie
                    ]

                    # rankings_with_tie without r1 and r2
                    other_rankings_with_tie = remove_first_occurrences(
                        rankings_with_tie, r1, r2
                    )

                    new_rankings = (
                        [r1.break_tie(lin_order), r2.break_tie(reverse_lin_order)]
                        + other_rankings_with_tie
                        + other_rankings
                    )
            ...
            for r1, r2 in combinations(rankings_with_tie, 2):
                ...
                for lin_order, reverse_lin_order in linear_orders_with_reverse(cands):
                    ...
                    if vm(prof) != vm(new_prof):
                        ...
                        violations.append(
                            (
                                (r1, r1.break_tie(lin_order)),
                                (r2, r2.break_tie(reverse_lin_order)),
                            )
                        )
    return violations
