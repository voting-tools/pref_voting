...
def _support(ranks, rcounts, c1, c2):
    ...
    diffs = (
        ranks[0:, c1] - ranks[0:, c2]
    )  # for each voter, the difference of the ranks of c1 and c2
    diffs[diffs > 0] = 0  # c1 is ranked below c2
    diffs[diffs < 0] = 1  # c1 is ranked above c2
    num_rank_c1_over_c2 = np.multiply(
        diffs, rcounts
    )  # mutliply by the number of each ranking
...
def _borda_score(rankings, rcounts, num_cands, cand):
    ...
    num_ranks = np.array(
        [_num_rank(rankings, rcounts, cand, level) for level in levels]
    )
...
def _find_updated_profile(rankings, cands_to_ignore, num_cands):
    ...
    updated_prof_ranks = np.empty(
        shape=(rankings.shape[0], updated_cand_num), dtype=np.int32
    )
...
class Profile(object):
    ...
    def __init__(self, rankings, rcounts=None, cmap=None):
        ...
        assert all([all([c in self.candidates for c in r]) for r in rankings]), (
            f"The candidates must be from the set {self.candidates}."
        )

        # for number of each ranking
        self._rcounts = (
            np.array([1] * len(rankings)) if rcounts is None else np.array(rcounts)
        )

        # for each voter, the ranks of each candidate
        self._ranks = np.array(
            [
                [np.where(_r == c)[0][0] + 1 for c in self.candidates]
        ...
        self._tally = np.array(
            [
                [_support(self._ranks, self._rcounts, c1, c2) for c2 in self.candidates]
    ...
    def rankings(self):
        ...
        return [
            ...
            for n in range(self._rcounts[ridx])
        ]
    ...
    def plurality_scores(self, curr_cands=None):
        ...
        return {
            c: _num_rank_first(rankings, rcounts, cands_to_ignore, c)
            for c in curr_cands
        }
    ...
    def borda_scores(self, curr_cands=None):
        ...
        rankings = (
            ...
            if curr_cands is None
                ...
                self._rankings,
                np.array([c for c in self.candidates if c not in curr_cands]),
                len(self.candidates),
            )
        )
        curr_cands = self.candidates if curr_cands is None else curr_cands
        ...
        return {
            c: _borda_score(rankings, self._rcounts, num_cands, c) for c in curr_cands
        }
    ...
    def remove_candidates(self, cands_to_ignore):
        ...
        """
        updated_rankings = _find_updated_profile(
            self._rankings, np.array(cands_to_ignore), self.num_cands
        )
        new_names = {c: cidx for cidx, c in enumerate(sorted(updated_rankings[0]))}
        orig_names = {v: k for k, v in new_names.items()}
        return Profile(
            [[new_names[c] for c in r] for r in updated_rankings],
    ...
    def apply_cand_permutation(self, perm):
        ...
        assert all(c in self.candidates for c in perm.keys()), (
            "All keys must be valid candidates"
        )
        assert all(c in self.candidates for c in perm.values()), (
            "All values must be valid candidates"
        )
        assert (
            len(set(perm.keys())) == len(set(perm.values())) == len(self.candidates)
    ...
    def to_profile_with_ties(self):
        ...
        return ProfileWithTies(
            [{c: cidx for cidx, c in enumerate(list(r))} for r in ranks],
            rcounts=list(rcounts),
            candidates=self.candidates,
            cmap=self.cmap,
        )

    def randomly_truncate(self, truncation_prob_list=None, seed=None):
        ...
        for ranking, count in zip(*self.rankings_counts):
            for _ in range(count):
                random_number_of_cands_ranked = rng.choice(
                    np.arange(1, self.num_cands + 1), p=truncation_prob_list
                )
                truncated_ranking = ranking[:random_number_of_cands_ranked]
                new_ballot = {cand: pos for pos, cand in enumerate(truncated_ranking)}
    ...
    def to_utility_profile(self, seed=None):
        ...
        return UtilityProfile(
            [
                Utility.from_linear_ranking(
                    r, seed=(seed + idx if seed is not None else None)
                )
    ...
    def to_latex(self, cmap=None, curr_cands=None):
        ...
        latex_str += "\\\\ \n".join(
            [" & ".join([f"${cmap[c]}$" for c in cs]) for cs in rankings.transpose()]
        )
        latex_str += "\n\\end{tabular}"
    ...
    def display_margin_graph_with_defeat(
        ...
        MarginGraph.from_profile(self).display_with_defeat(
            defeat, curr_cands=curr_cands, show_undefeated=show_undefeated, cmap=cmap
        )
    ...
    def display(
        self, cmap=None, style="pretty", curr_cands=None, order_by_counts=False
    ):
        ...
        if order_by_counts:
            rankings, rcounts = zip(
                *sorted(zip(rankings, self._rcounts), key=lambda x: x[1], reverse=True)
            )
        ...
        print(
            tabulate(
                [[cmap[c] for c in cs] for cs in rankings.transpose()],
                rcounts,
                tablefmt=style,
            )
        )
    ...
    def from_preflib(cls, instance_or_preflib_file, include_cmap=False):
        ...
        return preflib_to_profile(
            instance_or_preflib_file, include_cmap=include_cmap, as_linear_profile=True
        )
    ...
    def __add__(self, other_prof):
        ...
        return Profile(
            np.concatenate([self._rankings, other_prof._rankings]),
            rcounts=np.concatenate([self._rcounts, other_prof._rcounts]),
        )
    ...
    def __str__(self):
        ...
        return tabulate(
            [[self.cmap[c] for c in cs] for cs in self._rankings.transpose()],
            self._rcounts,
            tablefmt="pretty",
        )
