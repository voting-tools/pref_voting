...
from pref_voting.profiles import Profile, _find_updated_profile, _num_rank
...
@vm(
    name="Plurality",
    input_types=[ElectionTypes.PROFILE, ElectionTypes.TRUNCATED_LINEAR_PROFILE],
)
...
def plurality_ranking(profile, curr_cands=None, local=True, tie_breaking=None):
    ...
    if local:
        ...
        plurality_scores_dict = {
            k: v for k, v in plurality_scores_dict.items() if k in cands
        }
...
@vm(name="Borda", input_types=[ElectionTypes.PROFILE, ElectionTypes.MARGIN_GRAPH])
def borda(edata, curr_cands=None, algorithm="positional"):
    ...
    if algorithm == "marginal":
        borda_scores = {
            c: sum([edata.margin(c, d) for d in curr_cands]) for c in curr_cands
        }
...
@vm(name="Anti-Plurality", input_types=[ElectionTypes.PROFILE])
def anti_plurality(profile, curr_cands=None):
    ...
    last_place_scores = {
        c: _num_rank_last(rankings, rcounts, cands_to_ignore, c) for c in curr_cands
    }
    min_last_place_score = min(list(last_place_scores.values()))

    return sorted(
        [c for c in curr_cands if last_place_scores[c] == min_last_place_score]
    )
...
def anti_plurality_ranking(profile, curr_cands=None, local=True, tie_breaking=None):
    ...
    anti_plurality_scores_dict = {
        c: _num_rank_last(rankings, rcounts, cands_to_ignore, c) for c in cands
    }
...
def scoring_rule(
    profile, curr_cands=None, score=lambda num_cands, rank: 1 if rank == 1 else 0
):
    ...
    rankings = (
        ...
        if curr_cands is None
            ...
            np.array(_rankings), cands_to_ignore, len(profile.candidates)
        )
    )
    ...
    cand_scores = {
        c: sum(
            _num_rank(rankings, rcounts, c, level) * score(len(candidates), level)
            for level in range(1, len(candidates) + 1)
        )
        for c in candidates
    }
...
@vm(name="Dowdall", input_types=[ElectionTypes.PROFILE])
def dowdall(profile, curr_cands=None):
    ...
    return scoring_rule(
        profile, curr_cands=curr_cands, score=lambda num_cands, rank: 1 / rank
    )


@vm(name="Positive-Negative Voting", input_types=[ElectionTypes.PROFILE])
def positive_negative_voting(profile, curr_cands=None):
    ...
    return scoring_rule(
        ...
        score=lambda num_cands, rank: (
            1 if rank == 1 else (-1 if rank == num_cands else 0)
        ),
    )
        ...
        cand: sum(
            [
                len(
                    [
                        _cand
                        for _cand in profile.candidates
                        if r.extended_strict_pref(cand, _cand)
                    ]
                )
                * c
            ...
            [
                len(
                    [
                        _cand
                        for _cand in profile.candidates
                        if r.extended_strict_pref(_cand, cand)
                    ]
                )
                * c
                for r, c in zip(*profile.rankings_counts)
            ...
            [
                len(
                    [
                        _cand
                        for _cand in profile.candidates
                        if r.extended_weak_pref(cand, _cand)
                    ]
                )
                * c
                for r, c in zip(*profile.rankings_counts)
...
@vm(
    name="Borda (for Truncated Profiles)",
    input_types=[ElectionTypes.TRUNCATED_LINEAR_PROFILE],
)
def borda_for_profile_with_ties(
    profile, curr_cands=None, borda_scores=symmetric_borda_scores
):
    """
    ...
    restricted_prof = profile.remove_candidates(
    ...
    return sorted(
        [c for c in restricted_prof.candidates if b_scores[c] == max_borda_score]
    )


scoring_swfs = [plurality_ranking, borda_ranking, anti_plurality_ranking, score_ranking]
