...
from pref_voting.profiles_with_ties import ProfileWithTies, _num_rank_profile_with_ties
...
@vm(name="Pareto", input_types=[ElectionTypes.PROFILE, ElectionTypes.PROFILE_WITH_TIES])
def pareto(
    profile, curr_cands=None, strong_Pareto=False, use_extended_strict_preferences=True
):
    ...
    if type(profile) == ProfileWithTies:
        currently_using_extended_strict_preferences = (
            profile.using_extended_strict_preference
        )
    ...
    for a in candidates:
        for b in candidates:
            ...
            if (
                strong_Pareto
                and profile.support(a, b) > 0
                and profile.support(b, a) == 0
            ):
                Pareto_dominated.add(b)
...
def kendalltau_dist_for_rankings_with_ties(candidates, ranking1, ranking2, penalty=0.5):
    ...
    for c1, c2 in combinations(candidates, 2):
        ...
        if (
            ranking1.extended_strict_pref(c1, c2)
            and ranking2.extended_strict_pref(c2, c1)
        ) or (
            ranking1.extended_strict_pref(c2, c1)
            and ranking2.extended_strict_pref(c1, c2)
        ):
            tau += 1
        elif (
            ranking1.extended_strict_pref(c1, c2) and ranking2.extended_indiff(c1, c2)
        ) or (
            ranking1.extended_strict_pref(c2, c1) and ranking2.extended_indiff(c1, c2)
        ):
            tau += penalty
        elif (
            ranking1.extended_indiff(c1, c2) and ranking2.extended_strict_pref(c1, c2)
        ) or (
            ranking1.extended_indiff(c1, c2) and ranking2.extended_strict_pref(c2, c1)
        ):
            tau += penalty
...
def _kemeny_young_rankings(rankings, rcounts, candidates):
    ...
    for ranking in permutations(candidates):
        rankings_dist[tuple(ranking)] = sum(
            c * kendalltau_dist(tuple(r), ranking) for r, c in zip(rankings, rcounts)
        )
...
def kemeny_young_rankings(profile, curr_cands=None):
    ...
    rankings = (
        ...
        else _find_updated_profile(
            profile._rankings,
            np.array([c for c in profile.candidates if c not in curr_cands]),
            profile.num_cands,
        )
    )
...
def kemeny_young(edata, curr_cands=None, algorithm="marginal"):
    ...
    """
    assert algorithm in ["marginal", "Kendall tau"], (
        "Algorithm must be either 'marginal' or 'Kendall tau'."
    )
    ...
    if algorithm == "Kendall tau":
        rankings = (
            ...
            if curr_cands is None
                ...
                edata._rankings,
                np.array([c for c in edata.candidates if c not in curr_cands]),
                edata.num_cands,
            )
        )
        ky_rankings, min_dist = _kemeny_young_rankings(
            list(rankings), list(edata._rcounts), candidates
        )
...
@vm("Preliminary Weighted Condorcet", input_types=[ElectionTypes.PROFILE])
def preliminary_weighted_condorcet(
    prof, curr_cands=None, show_orders=False, require_positive_plurality_score=False
):
    ...
    if require_positive_plurality_score:
        assert all(
            [prof.plurality_scores(curr_cands=curr_cands)[c] > 0 for c in cands]
        ), "All candidates must have a positive plurality score."
    ...
    for r in permutations(cands):
        ...
        for i in r[:-1]:
            for j in r[r.index(i) + 1 :]:
                score_of_r += (
                    prof.plurality_scores(curr_cands=curr_cands)[i]
                    * prof.plurality_scores(curr_cands=curr_cands)[j]
                    * prof.margin(i, j)
                )
...
@vm(name="Bucklin", input_types=[ElectionTypes.PROFILE])
def bucklin(profile, curr_cands=None):
    ...
    for r in ranks:
        cand_to_num_voters_rank[r] = {
            c: _num_rank(rankings, rcounts, c, r) for c in candidates
        }
        cand_scores = {
            c: sum(
                [
                    cand_to_num_voters_rank[_r][c]
                    for _r in cand_to_num_voters_rank.keys()
                ]
            )
            for c in candidates
        }
...
@vm(
    name="Bucklin for Truncated Linear Orders",
...
def bucklin_for_truncated_linear_orders(profile, curr_cands=None):
    ...
    """
    assert all([not r.has_overvote() for r in profile.rankings]), (
        "Bucklin is only defined when all the ballots are truncated linear orders."
    )
    ...
    if curr_cands is not None:
        working_profile = working_profile.remove_candidates(
            [c for c in working_profile.candidates if c not in curr_cands]
        )
    ...
    for r in range(1, num_cands + 1):
        cand_to_num_voters_rank[r] = {
            c: _num_rank_profile_with_ties(rankings, rcounts, c, r) for c in candidates
        }
        # Calculate cumulative scores up to current rank
        cand_scores = {
            c: sum([cand_to_num_voters_rank[_r][c] for _r in range(1, r + 1)])
            for c in candidates
        }
...
@vm(name="Simplified Bucklin", input_types=[ElectionTypes.PROFILE])
...
def simplified_bucklin_with_explanation(profile, curr_cands=None):
    ...
    return sorted(
        [c for c in candidates if cand_scores[c] >= strict_maj_size]
    ), cand_scores


@vm(name="Weighted Bucklin", input_types=[ElectionTypes.PROFILE])
def weighted_bucklin(
    profile,
    curr_cands=None,
    strict_threshold=False,
    score=lambda num_cands, rank: (
        (num_cands - rank) / (num_cands - 1) if num_cands > 1 else 1
    ),
):
    r"""The Weighted Bucklin procedure, studied by D. Marc Kilgour, Jean-Charles Grégoire, and Angèle Foley. The k-th Weighted Bucklin score of a candidate c is the sum for j \leq k of the product of score(num_cands,j) and the number of voters who rank c in j-th place. Compute higher-order Weighted Bucklin scores until reaching a k such that some candidate's k-th Weighted Bucklin score is at least half the number of voters (or the strict majority size if strict_threshold = True). Then return the candidates with maximal k-th Weighted Bucklin score. Bucklin is the special case where strict_threshold = True and score = lambda num_cands, rank: 1.
    ...
    for r in ranks:
        ...
        cand_scores = {
            c: sum(
                [
                    score(len(candidates), _r) * cand_to_num_voters_rank[_r][c]
                    for _r in cand_to_num_voters_rank.keys()
                ]
            )
            for c in candidates
...
def _dodgson_score(profile, cand):
    ...
    *Approximability and computational feasibility of Dodgson’s rule*.
    ...
    solver.Minimize(solver.Sum(k * var for (r_idx, k), var in x.items() if k > 0))
...
@vm(name="Dodgson", input_types=[ElectionTypes.PROFILE])
def dodgson(profile, curr_cands=None, global_score=True):
    ...
    if not global_score:
        profile.remove_candidates(
            [c for c in profile.candidates if c not in curr_cands]
        )
...
@vm(name="Bracket Voting", input_types=[ElectionTypes.PROFILE])
def bracket_voting(profile, curr_cands=None, seed=None, tie_break="random"):
    ...
    potential_first_seeds = [
        c for c in cands if plurality_scores[c] == descending_plurality_scores[0]
    ]
    first_seed = min(
        potential_first_seeds, key=lambda c: tie_breaking_ordering.index(c)
    )

    potential_second_seeds = [
        c
        for c in cands
        if plurality_scores[c] == descending_plurality_scores[1] and c != first_seed
    ]
    second_seed = min(
        potential_second_seeds, key=lambda c: tie_breaking_ordering.index(c)
    )

    potential_third_seeds = [
        c
        for c in cands
        if plurality_scores[c] == descending_plurality_scores[2]
        and c not in [first_seed, second_seed]
    ]
    third_seed = min(
        potential_third_seeds, key=lambda c: tie_breaking_ordering.index(c)
    )

    potential_fourth_seeds = (
        [
            c
            for c in cands
            if plurality_scores[c] == descending_plurality_scores[3]
            and c not in [first_seed, second_seed, third_seed]
        ]
        if len(cands) > 3
        else []
    )
    fourth_seed = (
        min(potential_fourth_seeds, key=lambda c: tie_breaking_ordering.index(c))
        if len(potential_fourth_seeds) > 0
        else None
    )
    ...
    if len(cands) == 3:
        ...
        one_four_winner = (
            first_seed if profile.margin(first_seed, fourth_seed) >= 0 else fourth_seed
        )
        one_four_winner_seed = 1 if one_four_winner == first_seed else 4

    two_three_winner = (
        second_seed if profile.margin(second_seed, third_seed) >= 0 else third_seed
    )
    ...
    # Ties in the final head-to-head match are broken in favor of the higher-seeded candidate
        ...
        winner = (
            one_four_winner
            if one_four_winner_seed < two_three_winner_seed
            else two_three_winner
        )

    return [winner]


@vm(name="Superior Voting", input_types=[ElectionTypes.PROFILE])
...
@vm(name="Bradley-Terry", input_types=[ElectionTypes.PROFILE])
def bradley_terry(prof, curr_cands=None, threshold=0.00001):
...
def bradley_terry_ranking(prof, curr_cands=None, threshold=0.00001):
    ...
    while len(cands_assigned) < len(curr_cands):
        max_value = np.max(
            [params[curr_cands.index(c)] for c in curr_cands if c not in cands_assigned]
        )

        for c in curr_cands:
            if (
                c not in cands_assigned
                and np.abs(params[curr_cands.index(c)] - max_value) <= threshold
            ):
