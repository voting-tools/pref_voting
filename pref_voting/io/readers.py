...
def abif_to_profile(filename):
    ...
    with open(filename, mode="r") as file:
        ...
        for line in lines:
            if line.startswith("="):
                ctoken, cname = line[1:].strip().split(":")
                cmap[cindx] = cname.strip().strip("[]")
                # The ABIF token (before the ':') is the identifier used in the
                # vote lines, so the lookup table must be keyed by the token, not
                # the display name.
                cand_to_indices[ctoken.strip()] = cindx
                cindx += 1
        ...
        for line in lines:
            if line.startswith("#"):
                ...
                assert not any(["=" in cs or "," in cs for cs in ranking]), (
                    "The election must contain linear orders on the candidates to create a Profile."
                )

                if len(cmap) == 0:
                    # no candidate map provided, so need to create one from the rankings
                    cmap = {
                        cidx: str(sorted(ranking)[cidx].strip())
                        for cidx in range(len(sorted(ranking)))
                    }
                    cand_to_indices = {c: i for i, c in cmap.items()}

                r = list()
                assert len(cmap) > 0 and len(ranking) == len(cmap), (
                    "The election must contain linear orders on the candidates to create a Profile."
                )
                for c in ranking:
                    assert len(cmap) > 0 and c in cand_to_indices.keys(), (
                        "Candidate found that is not in the candidate map."
                    )
    ...
    with open(filename, mode="r") as file:
        ...
        for line in lines:
            if line.startswith("#"):
                ...
                cmap[
                    cand_type(cidx.strip()) if cand_type is not None else cidx.strip()
                ] = cname.strip().strip("[]")
            elif line.startswith("{"):
                # metadata
                continue
        ...
        if len(cmap) == 0:
            return ProfileWithTies(rankings, rcounts=rcounts)
        else:
...
def preflib_to_profile(
    ...
    assert (
        type(instance_or_preflib_file) == OrdinalInstance
        or type(instance_or_preflib_file) == str
    ), "The argument must be an instance of OrdinalInstance or a string."

    if type(instance_or_preflib_file) == str:
        preflib_file = instance_or_preflib_file

        assert (
            preflib_file.endswith(".soc")
            or preflib_file.endswith(".soi")
            or preflib_file.endswith(".toc")
            or preflib_file.endswith(".toi")
        ), (
            f"The file must be one of the file types from preflib: https://www.preflib.org/format#types, not {preflib_file}."
        )
    ...
    if not as_linear_profile:
        for order in instance.orders:
            ...
            for r, cs in enumerate(order):
                for c in cs:
                    ...
                    if include_cmap:
                        if use_cand_names:
                            cmap[instance.alternatives_name[c]] = (
                                instance.alternatives_name[c]
                            )
                        else:
        ...
        return ProfileWithTies(rankings, rcounts=rcounts, cmap=cmap)
...
def csv_to_profile(
    ...
    if csv_format == "rank_columns":
        ...
        if "writein" in cand_names:
            cands = list(set([c for c in sorted(cand_names) if c != "writein"])) + [
                "writein"
            ]
        else:
        ...
        for _, row in df.iterrows():
            ...
            ballot_dict = {
                cand_type(c) if cand_type is not None else c: r
        ...
        prof = ProfileWithTies(
            ...
            rcounts=rcounts,
            cmap={
                cand_type(c) if cand_type is not None else str(c): str(c)
                for c in candidates
            },
        )
        if as_linear_profile:
...
def json_to_profile(filename, cand_type=None, as_linear_profile=False):
    ...
    with open(filename, mode="r") as file:
        ...
        cmap = {
            _convert_key_type(c, candidates): c_str for c, c_str in data["cmap"].items()
        }
        ...
        for r_data in data["rankings"]:
            rank = {
                cand_type(c)
                if cand_type is not None
                else _convert_key_type(c, candidates): int(r)
                for c, r in r_data["ranking"].items()
            }
            rankings.append(rank)
    ...
    if as_linear_profile:
        prof = ProfileWithTies(
            rankings, rcounts=rcounts, candidates=candidates, cmap=cmap
        )
...
def read(
    ...
    if file_format == "abif":
        if as_linear_profile:
            return abif_to_profile(filename)
        else:
            return abif_to_profile_with_ties(filename, cand_type=cand_type)
    elif file_format == "json":
        return json_to_profile(
            filename, cand_type=cand_type, as_linear_profile=as_linear_profile
        )
...
def json_to_spatial_profile(filename):
    ...
    with open(filename, "r") as f:
        ...
        return SpatialProfile(
            {
                _convert_key_type(c, candidates): c_pos
                for c, c_pos in spatial_profile_dict["candidates"].items()
            },
            {
                _convert_key_type(v, voters): v_pos
                for v, v_pos in spatial_profile_dict["voters"].items()
            },
        )
