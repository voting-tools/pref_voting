...
class _Mapping(object):
    ...
    def __init__(
        ...
        assert domain is None or all([x in domain for x in mapping.keys()]), (
            f"Not all keys in {mapping} are in the domain {domain}."
        )

        assert domain is not None or all(
            [isinstance(v, (int, float)) for v in mapping.values()]
        ), f"Not all values in {mapping} are numbers."

        assert codomain is None or all([v in codomain for v in mapping.values()]), (
            f"Not all values in {mapping} are in the codomain {codomain}."
        )

        self.mapping = mapping
        self.domain = (
            domain if domain is not None else sorted(list(mapping.keys()))
        )  # if domain is None, then it is assumed to be all keys (so mapping is a total function)
        self.codomain = (
            codomain  # if codomain is None, then it is assumed to be any number
        )
        self.item_map = (
            item_map if item_map is not None else {x: str(x) for x in self.domain}
        )  # a dictionary mapping items in the domain to their names

        val_map = (
            val_map
            if val_map is not None
            else {v: str(v) for v in self.mapping.values()}
        )  # a dictionary mapping values to their names
        self.val_map = functools.partial(
            val_map_function, val_map=val_map
        )  # a function mapping values to their names

        self.compare_function = (
            compare_function
            if compare_function is not None
            else default_compare_function
        )
    ...
    def average(self, **kwargs):
        ...
        items = (
            ...
            or kwargs.get("candidates", None)
            or kwargs.get("alternatives", None)
        )

        assert items is None or all(
            [
                isinstance(self.val(x), (int, float)) or self.val(x) is None
                for x in items
            ]
    ...
    def compare(self, x, y):
        ...
        return (
            None
            if (not self.has_value(x) or not self.has_value(y))
            else self.compare_function(self.val(x), self.val(y))
        )
    ...
    def sorted_domain(self, extended=False):
        ...
        """
        indiff_classes = (
            self._indifference_classes(self.domain, use_extended=True)
            if extended
            else self._indifference_classes(self.defined_domain)
        )
...
class Utility(_Mapping):
    def __init__(self, utils, **kwargs):
        ...
        if "domain" in kwargs and "candidates" in kwargs:
            raise ValueError(
                "You can only provide either 'domain' or 'candidates', not both."
            )
        if "domain" in kwargs:
        ...
        self.cmap = (
            {x: str(x) for x in self.domain} if "cmap" not in kwargs else kwargs["cmap"]
        )

        assert self.domain is None or all([x in self.domain for x in utils.keys()]), (
            f"The domain {self.domain} must contain all elements in the utility map {utils}"
        )
    ...
    def to_approval_ballot(self, prob_to_cont_approving=1.0, decay_rate=0.0):
        ...
        main_approval_set = {
            x: self.val(x) for x in self.defined_domain if self.val(x) > avg_grade
        }

        sorted_approval_set = sorted(
            main_approval_set.items(), key=lambda a: a[1], reverse=True
        )
    ...
    def to_truncated_ranking(
        self, method, radius=None, min_gap=None, require_at_least_one=True
    ):
        """Return a truncated :class:`Ranking` of this utility function.

        The ballot ranks the voter's preferred candidates best-first (ties share a rank) and drops the rest. Both criteria are scale-free (they use only this voter's own utilities):

        * ``'radius'`` -- rank ``c`` iff ``u(best) - u(c) < radius``: every candidate within ``radius`` (in utility) of the voter's most-preferred candidate. The radius is measured from each voter's own favorite, so it is comparable across voters and across utility scales. Never abstains (the favorite is within radius 0); ``radius <= 0`` gives a bullet vote.
        * ``'gap'`` -- rank candidates best to worst, stopping as soon as two adjacent distinct utility levels are closer than ``min_gap`` (both members of the too-close pair, and everything below, are dropped). A voter whose top two levels are already within ``min_gap`` abstains unless ``require_at_least_one``is True.

        Args:
            method (str): the truncation criterion (``'radius'`` | ``'gap'``).
            radius (float): for ``'radius'`` -- rank ``c`` iff ``u(best) - u(c) < radius``.
            min_gap (float): for ``'gap'`` -- the smallest utility gap the voter can resolve.
            require_at_least_one (bool): if True (default), a voter who would rank no one still ranks their most-preferred candidate(s); if False, they abstain (an empty ranking).

        Returns:
            Ranking: ranks 1, 2, 3, ... (best first); ties share a rank; truncated candidates are absent from the ranking.
        """
        indiff_classes = self.sorted_domain()  # indifference classes, best first
        if len(indiff_classes) == 0:  # no candidate has a defined utility
            return Ranking({})
        indiff_class_util = [
            self(indifference_class[0])  # the utility of each class
            for indifference_class in indiff_classes
        ]

        if method == "radius":
            if radius is None:
                raise ValueError("method='radius' requires a 'radius' argument.")
            kept = [
                indifference_class
                for indifference_class, u in zip(indiff_classes, indiff_class_util)
                if indiff_class_util[0] - u < radius
            ]
        elif method == "gap":
            if min_gap is None:
                raise ValueError("method='gap' requires a 'min_gap' argument.")
            keep_count = len(
                indiff_classes
            )  # no too-close pair found -> keep every class
            for i in range(len(indiff_classes) - 1):
                if indiff_class_util[i] - indiff_class_util[i + 1] < min_gap:
                    keep_count = i  # stop before class i (too close to the next)
                    break
            kept = indiff_classes[:keep_count]
        else:
            raise ValueError(
                f"Unknown truncation method {method!r}: use 'radius' or 'gap'."
            )

        if len(kept) == 0:
            if not require_at_least_one:
                return Ranking({})
            kept = [indiff_classes[0]]  # the most-preferred candidate(s)

        return Ranking(
            {
                c: rank
                for rank, indifference_class in enumerate(kept, start=1)
    ...
    def ranking(self):
        ...
        return Ranking(
            {
                x: idx + 1
                for idx, indiff_class in enumerate(self.sorted_domain())
    ...
    def extended_ranking(self):
        ...
        return Ranking(
            {
                x: idx + 1
                for idx, indiff_class in enumerate(self.sorted_domain(extended=True))
                for x in indiff_class
            }
        )
    ...
    def represents_ranking(self, r, use_extended=False):
        ...
        if use_extended:
            for x in r.cands:
                for y in r.cands:
                    if r.extended_strict_pref(x, y) and not self.extended_strict_pref(
                        x, y
                    ):
                        return False
    ...
    def normalize_by_range(self):
        ...
        if max_util == min_util:
            return Utility(
                {x: 0 for x in self.defined_domain}, domain=self.domain, cmap=self.cmap
            )
        else:
            return Utility(
                {
                    x: (self.val(x) - min_util) / (max_util - min_util)
                    for x in self.defined_domain
                },
                domain=self.domain,
                cmap=self.cmap,
            )
    ...
    def normalize_by_standard_score(self):
        ...
        return Utility(
            {
                x: (self.val(x) - mean_utility) / std_dev_utility
                for x in self.defined_domain
            },
            domain=self.domain,
            cmap=self.cmap,
    ...
    def expectation(self, prob):
        ...
        assert all([x in self.domain for x in prob.keys()]), (
            "The domain of the probability distribution must be a subset of the domain of the utility function."
        )
        ...
        return sum(
            [
                ...
                if x in prob.keys() and self.has_utility(x)
            ]
        )
...
class Grade(_Mapping):
    def __init__(
        ...
        assert all([g in grades for g in grade_map.values()]), (
            f"All the grades in the grade map {grade_map} must be in the grades {grades}"
        )
        assert candidates is None or all([x in candidates for x in grade_map.keys()]), (
            f"The candidates {candidates} must contain all elements in the grade map {grade_map}"
        )

        self.candidates = (
            sorted(candidates)
            if candidates is not None
            else sorted(list(grade_map.keys()))
        )
