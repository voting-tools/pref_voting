...
def minimax(edata, curr_cands = None, strength_function = None):   
    ...
    scores = {c: max([strength_function(_c, c) for _c in edata.dominators(c) if _c in candidates]) if len([_c for _c in edata.dominators(c) if _c in candidates]) > 0 else 0 
              for c in candidates}
