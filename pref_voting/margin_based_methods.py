...
from pref_voting.helper import SPO, get_mg
...
@vm(
    name="Minimax",
    input_types=[
        ElectionTypes.PROFILE,
        ElectionTypes.PROFILE_WITH_TIES,
        ElectionTypes.MARGIN_GRAPH,
    ],
)
def minimax(edata, curr_cands=None, strength_function=None):
    ...
    scores = {
        c: max(
            [strength_function(_c, c) for _c in edata.dominators(c) if _c in candidates]
        )
        if any([_c in edata.dominators(c) for _c in candidates])
        else 0
        for c in candidates
    }
...
def minimax_scores(edata, curr_cands=None, score_method="margins"):
    ...
    score_functions = {
        "winning": lambda cs, c: (
            max([edata.support(_c, c) for _c in cs]) if len(cs) > 0 else 0
        ),
        "margins": lambda cs, c: (
            max([edata.margin(_c, c) for _c in cs]) if len(cs) > 0 else 0
        ),
    ...
    return {
        c: -1 * score_functions[score_method](cands[score_method](c), c)
        for c in candidates
    }
...
def leximax(edata, curr_cands=None):
    ...
    sequence_of_margins_dict = {
        cand: sorted(
            [
                edata.margin(other_cand, cand)
    ...
    candidates_with_minimal_sequence_of_margins = [
        cand
        for cand in curr_cands
        if sequence_of_margins_dict[cand] == min(sequence_of_margins_dict.values())
    ]
...
def MWSL(
    edata, half_point_for_ties=True, lexicographic_tie_breaker=True, curr_cands=None
):
    ...
    if not lexicographic_tie_breaker:
        ...
        for c in most_wins:
            ...
            if len(defeaters_of_c) > 0:
                smallest_loss_dict[c] = min(
                    [edata.margin(d, c) for d in defeaters_of_c]
                )
            else:
                smallest_loss_dict[c] = (
                    0  # if c has no loss, we count their "smallest loss" as 0
                )
        ...
        return [
            c
            for c in most_wins
            if smallest_loss_dict[c] == min(smallest_loss_dict.values())
        ]
        ...
        for c in most_wins:
            defeaters_of_c = edata.dominators(c, curr_cands=curr_cands)
            if len(defeaters_of_c) > 0:
                sequence_of_losses_dict[c] = sorted(
                    [edata.margin(d, c) for d in defeaters_of_c], reverse=True
                )
            else:
        ...
        return [
            c
            for c in most_wins
            if sequence_of_losses_dict[c] == min(sequence_of_losses_dict.values())
        ]
...
def _beat_path_basic(edata, curr_cands=None, strength_function=None):
    ...
    beat_paths_weights = {
        c: {c2: 0 for c2 in candidates if c2 != c} for c in candidates
    }
    for c in candidates:
        for other_c in beat_paths_weights[c].keys():
            all_paths = list(nx.all_simple_paths(mg, c, other_c))
            if len(all_paths) > 0:
                beat_paths_weights[c][other_c] = max(
                    [
                        min(
                            [
                                strength_function(p[i], p[i + 1])
    ...
    for c in candidates:
        if all(
            [
                beat_paths_weights[c][c2] >= beat_paths_weights[c2][c]
...
def _beat_path_floyd_warshall(edata, curr_cands=None, strength_function=None):
        ...
        for j_idx, j in enumerate(candidates):
            if i != j:
                for k_idx, k in enumerate(candidates):
                    if i != k and j != k:
                        strength[j_idx][k_idx] = max(
                            strength[j_idx][k_idx],
                            min(strength[j_idx][i_idx], strength[i_idx][k_idx]),
                        )
...
def _schwartz_sequential_dropping(edata, curr_cands=None, strength_function=None):
    ...
    pos_schwartz_strengths = [
        strength_function(c, d)
        for c in schwartz
        for d in schwartz
        if strength_function(c, d) > 0
    ]
    ...
    if max_schwartz_strength == min_schwartz_strength:
        ...
        new_mg = MarginGraph(
            schwartz,
            [
                (c, d, strength_function(c, d))
                for c in schwartz
                for d in schwartz
                if strength_function(c, d) > min_schwartz_strength
            ],
        )
...
def beat_path(
    edata, curr_cands=None, strength_function=None, algorithm="floyd_warshall"
):
    r"""For candidates :math:`a` and :math:`b`, a **path** from :math:`a` to :math:`b` is a sequence
    ...
    if algorithm == "floyd_warshall":
        return _beat_path_floyd_warshall(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    elif algorithm == "basic":
        return _beat_path_basic(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    elif algorithm == "schwartz_sequential_dropping":
        return _schwartz_sequential_dropping(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    else:
...
def beat_path_defeat(edata, curr_cands=None, strength_function=None):
    ...
    for i_idx, i in enumerate(candidates):
        for j_idx, j in enumerate(candidates):
            if i != j:
                if strength[j_idx][i_idx] > strength[i_idx][j_idx]:
                    defeat_graph.add_weighted_edges_from(
                        [(j, i, s_matrix[j_idx][i_idx])]
                    )
...
def _split_cycle_basic(edata, curr_cands=None, strength_function=None):
    """An implementation of Split Cycle based on the mathematical definition."""
    strength_matrix, cand_to_cindex = edata.strength_matrix(
        curr_cands=curr_cands, strength_function=strength_function
    )
    ...
    for a in candidates:
        for b in candidates:
            if strength_function(b, a) > strength_function(
                a, b
            ) and not has_strong_path(
                strength_matrix,
                cand_to_cindex(a),
                cand_to_cindex(b),
                strength_function(b, a),
            ):
...
def _is_cand_split_cycle_defeated(a, strength_matrix):
    ...
    for b in range(strength_matrix.shape[0]):
        if strength_matrix[b][a] > strength_matrix[a][b] and not has_strong_path(
            strength_matrix, a, b, strength_matrix[b][a]
        ):
            return True
    return False
...
def _split_cycle_basic_parallel(strength_matrix, num_cpus=4):
    ...
    with Pool(num_cpus) as pool:
        batch_args = [(batch, strength_matrix) for batch in candidate_batches]
...
def _split_cycle_floyd_warshall(edata, curr_cands=None, strength_function=None):
    ...
    for c1_idx, c1 in enumerate(candidates):
        for c2_idx, c2 in enumerate(candidates):
            if edata.majority_prefers(c1, c2) or c1 == c2:
                s_matrix[c1_idx][c2_idx] = strength_function(c1, c2)
                weak_condorcet_winners[c2] = weak_condorcet_winners[c2] and (
                    c1 == c2
    ...
    for i_idx, i in enumerate(candidates):
        for j_idx, j in enumerate(candidates):
            if i != j:
                if (
                    s_matrix[j_idx][i_idx] > strength[i_idx][j_idx]
                ):  # the main difference with Beat Path
                    winners[i] = False
    return sorted([c for c in candidates if winners[c]])
...
def split_cycle(
    edata, curr_cands=None, strength_function=None, algorithm="basic", num_cpus=4
):
    r"""A **majority cycle** is a sequence :math:`x_1, \ldots ,x_n` of distinct candidates with :math:`x_1=x_n` such that for :math:`1 \leq k \leq n-1`,  :math:`x_k` is majority preferred to :math:`x_{k+1}`.  The Split Cycle winners are determined as follows:
    ...
    if algorithm == "basic":
        return _split_cycle_basic(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    elif algorithm == "floyd_warshall":
        return _split_cycle_floyd_warshall(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
...
def is_stack(edata, cand_list, curr_cands=None):
    ...
    cand_pairs = [
        (a, b) if cand_list.index(a) < cand_list.index(b) else (b, a)
    ...
    for a, b in cand_pairs:
        ...
        for indices in powerset(range(len(sublist))):
            ...
            if all(
                [
                    cand_list.index(xi) < cand_list.index(path[i + 1])
                    for i, xi in enumerate(path[0:-1])
                ]
            ) and all([m >= edata.margin(b, a) for m in margins]):
                found_path = True
                break
        if not found_path:
            return False
...
def _tie_breaking_orders(sorted_edges):
    """Lazily enumerate the tie-breaking orders of the edges: the product of the permutations of each equal-strength edge group.

    itertools.product materializes each permutations(...) pool up front, which requires k! tuples for a group of k tied edges and can exhaust memory; this generator yields the same tuples in the same order using only O(number of groups) memory.
    """
    if not sorted_edges:
        yield ()
        return
    for first in permutations(sorted_edges[0]):
        for rest in _tie_breaking_orders(sorted_edges[1:]):
            yield (first,) + rest
...
def _ranked_pairs_basic(edata, curr_cands=None, strength_function=None):
    ...
    if cw is not None:
        ...
        if len(w_edges) > 0:
            ...
            tbs = _tie_breaking_orders(sorted_edges)
            for tb in tbs:
                edges = flatten(tb)
...
def ranked_pairs(edata, curr_cands=None, strength_function=None, algorithm="basic"):
    """
...
@vm(name="Ranked Pairs", skip_registration=True)
...
def ranked_pairs_defeats(
    edata, curr_cands=None, strength_function=None, add_reverse_of_removed_edges=False
):
    """
...
def ranked_pairs_tb(edata, curr_cands=None, tie_breaker=None, strength_function=None):
    ...
    if cw is not None:
        ...
        for s in strengths:
            ...
            sorted_edges = sorted(
                edges,
                key=lambda e: (tb_ranking.index(e[0]), tb_ranking.index(e[1])),
                reverse=False,
            )
...
@vm(name="Ranked Pairs ZT", input_types=[ElectionTypes.PROFILE])
def ranked_pairs_zt(profile, curr_cands=None, strength_function=None):
    ...
    return ranked_pairs_tb(
        profile,
        curr_cands=curr_cands,
        tie_breaker=tb_ranking,
        strength_function=strength_function,
    )


def ranked_pairs_defeat_tb(
    edata, curr_cands=None, tie_breaker=None, strength_function=None, return_list=False
):
    """
...
def river(edata, curr_cands=None, strength_function=None):
    ...
    if cw is not None:
        ...
        for tb in tbs:
            ...
            for e0, e1, s in edges:
                if (
                    not rv_defeat.P[cand_to_cidx[e1]][cand_to_cidx[e0]]
                    and len(rv_defeat.preds[cand_to_cidx[e1]]) == 0
                ):
...
def river_defeats(edata, curr_cands=None, strength_function=None):
    ...
    for tb in tbs:
        ...
        for e in edges:
            if (
                e[1] not in river_defeat.nodes
                or len(list(river_defeat.in_edges(e[1]))) == 0
            ):
...
@vm(name="River", skip_registration=True)
...
@vm(name="River ZT", input_types=[ElectionTypes.PROFILE])
...
def _simple_stable_voting(curr_cands, sorted_matches, mem_sv_winners):
    """
    ...
    for a, b, s in sorted_matches:
        ...
        if a not in sv_winners:
            ...
            if cands_minus_b_key not in mem_sv_winners.keys():
                ws, mem_sv_winners = _simple_stable_voting(
                    curr_cands=cands_minus_b,
                    sorted_matches=[
                        (a, c, s) for a, c, s in sorted_matches if a != b and c != b
                    ],
...
def _simple_stable_voting_with_condorcet_check(
    edata, curr_cands=None, strength_function=None
):
    ...
    if cw is not None:
        ...
        curr_cands = edata.candidates if curr_cands is None else curr_cands
        strength_function = (
            edata.margin if strength_function is None else strength_function
        )

        matches = [
            (a, b, strength_function(a, b))
            for a in curr_cands
            for b in curr_cands
            if a != b
        ]
        sorted_matches = sorted(
            matches, reverse=True, key=lambda m_w_weight: m_w_weight[2]
        )

        return sorted(
            _simple_stable_voting(
                curr_cands=curr_cands, sorted_matches=sorted_matches, mem_sv_winners={}
            )[0]
        )
...
def simple_stable_voting(
    edata, curr_cands=None, strength_function=None, algorithm="basic"
):
    """Implementation of Simple Stable Voting from https://arxiv.org/abs/2108.00542.
    ...
    if algorithm == "basic":
        return _simple_stable_voting_basic(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    elif algorithm == "with_condorcet_check":
        return _simple_stable_voting_with_condorcet_check(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    else:
        raise ValueError("Invalid algorithm specified.")
...
def _simple_stable_voting_with_explanation(
    curr_cands, sorted_matches, mem_sv_winners, mem_elim_dict
):
    """
    Determine the Simple Stable Voting winners while keeping track
    of the winners in any subprofiles checked during computation
        ...
        if a not in sv_winners:
            ...
            if cands_minus_b_key not in mem_sv_winners.keys():
                ws, mem_sv_winners, elim_dict, mem_elim_dict = (
                    _simple_stable_voting_with_explanation(
                        curr_cands=cands_minus_b,
                        sorted_matches=[
                            (a, c, s) for a, c, s in sorted_matches if a != b and c != b
                        ],
...
def _simple_stable_voting_basic_with_explanation(
    edata, curr_cands=None, strength_function=None
):
    """Implementation of Simple Stable Voting from https://arxiv.org/abs/2108.00542.
...
def simple_stable_voting_with_explanation(
    ...
    if algorithm == "basic":
        return _simple_stable_voting_basic_with_explanation(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    elif algorithm == "with_condorcet_check":
        return _simple_stable_voting_with_condorcet_check_with_explanation(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    else:
        raise ValueError("Invalid algorithm specified.")
...
def _stable_voting(
    ...
    if (
        favor_weak_condorcet_winners
        and len(edata.weak_condorcet_winner(curr_cands=curr_cands)) > 0
    ):
        undefeated_candidates = edata.weak_condorcet_winner(curr_cands=curr_cands)
    else:
        undefeated_candidates = split_cycle(
            edata, curr_cands=curr_cands, strength_function=strength_function
        )
    ...
    for a, b, s in sorted_matches:
        ...
        if a in undefeated_candidates and a not in sv_winners:
            ...
            if cands_minus_b_key not in mem_sv_winners:
                ws, mem_sv_winners = _stable_voting(
                    edata,
                    curr_cands=cands_minus_b,
                    strength_function=strength_function,
                    sorted_matches=[
                        (x, y, s) for x, y, s in sorted_matches if x != b and y != b
                    ],
                    mem_sv_winners=mem_sv_winners,
...
def _stable_voting_with_explanation(
    edata,
    curr_cands,
    strength_function,
    sorted_matches,
    ...
    if terminate_early and len(undefeated_candidates) == 1:
        mem_sv_winners[tuple(undefeated_candidates)] = undefeated_candidates
        mem_elim_dict[tuple(undefeated_candidates)] = {
            c: list() for c in undefeated_candidates
        }
        return (
            undefeated_candidates,
            mem_sv_winners,
            {c: list() for c in undefeated_candidates},
            mem_elim_dict,
        )
    ...
    for a, b, s in sorted_matches:
        ...
        if a in undefeated_candidates and a not in sv_winners:
            ...
            if cands_minus_b_key not in mem_sv_winners.keys():
                ws, mem_sv_winners, elim_dict, mem_elim_dict = (
                    _stable_voting_with_explanation(
                        edata,
                        curr_cands=cands_minus_b,
                        strength_function=strength_function,
                        sorted_matches=[
...
def _stable_voting_with_condorcet_check_with_explanation(
    edata, curr_cands=None, strength_function=None, terminate_early=True
):
    """
    Stable Voting is Condorcet consistent. It is faster to skip executing the recursive algorithm when there is a Condorcet winner.
    ...
    if cw is not None:
        ...
        ws, mem_sv_winners, elim_dict, mem_elim_dict = _stable_voting_with_explanation(
            edata,
            curr_cands=curr_cands,
            strength_function=strength_function,
            sorted_matches=sorted_matches,
...
def stable_voting_with_explanation(
    ...
    if algorithm == "basic":
        return _stable_voting_basic_with_explanation(
            edata,
            curr_cands=curr_cands,
            strength_function=strength_function,
            terminate_early=False,
        ...
        return _stable_voting_with_condorcet_check_with_explanation(
            edata,
            curr_cands=curr_cands,
            strength_function=strength_function,
            terminate_early=False,
    ...
    for y in candidates:
        ...
        for x in candidates:
            if edata.support(x, y) / edata.num_voters >= beta:
                ...
                for z in candidates:
                    if (
                        edata.support(z, x) / edata.num_voters >= beta
                        and edata.support(z, y) / edata.num_voters < beta
                    ):
                        beta_covers = False
                        break
...
def loss_trimmer(edata, curr_cands=None):
    ...
    sum_of_margins_of_loss = {
        cand: sum(
            [
                edata.margin(other_cand, cand)
                for other_cand in curr_cands
                if edata.margin(other_cand, cand) > 0
            ]
        )
        for cand in curr_cands
    }
    ...
    biggest_losers = [
        cand
        for cand in curr_cands
        if sum_of_margins_of_loss[cand] == max_sum_of_margins_of_loss
    ]
    ...
    for bl in biggest_losers:
        winners_without_bl = loss_trimmer(
            edata, curr_cands=[cand for cand in curr_cands if cand != bl]
        )
...
def simplified_dodgson(edata, curr_cands=None):
    ...
    candidates_with_smallest_sum_of_margins_of_loss = [
        cand
        for cand in curr_cands
        if sum_of_margins_of_loss[cand] == smallest_sum_of_margins_of_loss
    ]
