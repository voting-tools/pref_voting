    ...
    for pm_name, pm in prob_models.items():
        for num_cands in numbers_of_candidates:
            for num_voters in numbers_of_voters:
                ...
                get_data = partial(
                    record_condorcet_efficiency_data, vms, num_cands, num_voters, pm
                )

                num_samples = 0
                error_ranges = []
                elect_condorcet_winner = {vm.name: [] for vm in vms}
                has_condorcet_winner = []
                while num_samples < min_num_samples or (
                    any([(err[1] - err[0]) > max_error for err in error_ranges])
                    ...
                    for d in data:
                        ...
                        if d["has_cw"]:
                            for vm in vms:
                                elect_condorcet_winner[vm.name].append(
                                    d["cw_winner"][vm.name]
                                )

                    error_ranges = [
                        binomial_confidence_interval(elect_condorcet_winner[vm.name])
                        if len(elect_condorcet_winner[vm.name]) > 0
                        else (0, np.inf)
                        for vm in vms
                    ]
                ...
                for vm in vms:
                    ...
                    data_for_df["condorcet_efficiency"].append(
                        np.mean(elect_condorcet_winner[vm.name])
                    )
                    err_interval = binomial_confidence_interval(
                    ...
                    data_for_df["percent_condorcet_winner"].append(
                        np.mean(has_condorcet_winner)
                    )
                    err_interval = binomial_confidence_interval(has_condorcet_winner)
                    data_for_df["percent_condorcet_winner_error"].append(
                        err_interval[1] - err_interval[0]
                    )
...
def record_num_winners_data(vms, num_cands, num_voters, probmod, probmod_param, t):
    ...
    prof = generate_profile(
        num_cands, num_voters, probmod=probmod, probmod_param=probmod_param
    )
...
def resoluteness_data(
    ...
    probmod_params_list = (
        [None] * len(probmods) if probmod_params is None else probmod_params
    )

    assert len(probmod_params_list) == len(probmods), (
        "probmod_params must be a list of the same length as probmods"
    )
        ...
        for num_cands in numbers_of_candidates:
            for num_voters in numbers_of_voters:
                ...
                for vm in vms:
                    ...
                    data_for_df["freq_multiple_winners"].append(
                        multiple_winners[vm.name] / num_trials
                    )
...
def record_axiom_violation_data(
    axioms, vms, num_cands, num_voters, probmod, verbose, t
):

    prof = generate_profile(num_cands, num_voters, probmod=probmod)

    return {
        ax.name: {vm.name: ax.has_violation(prof, vm, verbose=verbose) for vm in vms}
        for ax in axioms
    }
            ...
            for num_voters in numbers_of_voters:
                ...
                for ax in axioms:
                    for vm in vms:
                        ...
                        data_for_df["probmod"].append(probmod)
                        data_for_df["num_trials"].append(num_trials)
                        data_for_df["num_violations"].append(
                            sum([d[ax.name][vm.name] for d in data])
                        )
    print("Done.")
    return pd.DataFrame(data_for_df)
...
def estimated_variance_of_sampling_dist(
    values_for_each_experiment, mean_for_each_experiment=None
):
    # values_for_each_vm is a 2d numpy array

    mean_for_each_experiment = (
        np.nanmean(values_for_each_experiment, axis=1)
        if mean_for_each_experiment is None
        else mean_for_each_experiment
    )
    ...
    return np.where(
        ...
        (1 / (num_val_for_each_exp * (num_val_for_each_exp - 1)))
        * np.nansum((values_for_each_experiment - row_means_reshaped) ** 2, axis=1),
        np.nan,
    )
...
def estimated_std_error(values_for_each_experiment, mean_for_each_experiment=None):
    ...
    return np.sqrt(
        estimated_variance_of_sampling_dist(
            ...
            mean_for_each_experiment=mean_for_each_experiment,
        )
    )
...
def means_with_estimated_standard_error(
    ...
    if verbose:
        ...
        print(
            f"Remaining estimated standard errors greater than {max_std_error}:",
            np.sum(est_std_errors > max_std_error),
        )
        print(
            f"Estimated standard errors that are still greater than {max_std_error}:\n",
    ...
    while (
        np.isnan(est_std_errors).any()
        or np.any(est_std_errors > max_std_error)
        or (num_trials < min_num_trials)
    ) and (max_num_trials is None or num_trials < max_num_trials):
        if verbose:
