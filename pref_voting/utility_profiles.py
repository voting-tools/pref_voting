...
class UtilityProfile(object):
    ...
    def __init__(self, utilities, ucounts=None, domain=None, cmap=None):
        ...
        _domain = list(domain) if domain is not None else []
        for u in utilities:
    ...
    def util_sum(self, x):
        ...
        return (
            ...
            if self.has_utility(x)
            else None
        )

    def util_avg(self, x):
        """Return the average of the utilities of ``x`` over the voters that assign ``x`` a utility.
        If ``x`` is not assigned a utility by any voter, return None."""
        ...
        num = np.sum([c for u, c in zip(*self.utilities_counts) if u.has_utility(x)])
        return total / num
    ...
    def util_max(self, x):
        ...
        return (
            max([u(x) for u in self._utilities if u.has_utility(x)])
            if self.has_utility(x)
            else None
        )
    ...
    def util_min(self, x):
        ...
        return (
            min([u(x) for u in self._utilities if u.has_utility(x)])
            if self.has_utility(x)
            else None
        )
    ...
    def sum_utility_function(self):
        ...
        return Utility(
            {x: self.util_sum(x) for x in self.domain},
            domain=self.domain,
        )
    ...
    def avg_utility_function(self):
        ...
        return Utility(
            {x: np.average([u(x) for u in self.utilities]) for x in self.domain},
            domain=self.domain,
        )
        ...
        return GradeProfile(
            [
                u.to_approval_ballot(
                    prob_to_cont_approving=prob_to_cont_approving, decay_rate=decay_rate
                )
    ...
    def from_string(cls, uprof_str):
        ...
        num_alternatives, num_voters, utilities = (
            int(uprof_data[0]),
            int(uprof_data[1]),
            uprof_data[2:],
        )
        ...
        util_maps = [
            {int(cu.split(":")[0]): float(cu.split(":")[1]) for cu in utils.split(",")}
            if utils != ""
            else {}
            for utils in utilities
        ]
    ...
    def display(self, vmap=None, show_totals=False):
        ...
        vmap = (
            vmap
            if vmap is not None
            else {vidx: str(vidx + 1) for vidx in range(len(utilities))}
        )
        voters = range(len(utilities))

        if show_totals:
            tbl = {
                "Voter": [vmap[v] for v in voters]
                + [SEPARATING_LINE]
                + ["Sum", "Min", "Max"]
            }
            tbl.update(
                {
                    self.cmap[x]: [utilities[v](x) for v in voters]
                    + [SEPARATING_LINE]
                    + [self.util_sum(x), self.util_min(x), self.util_max(x)]
    ...
    def __setstate__(self, state):
        ...
        self._utilities = [
            Utility(u_dict, domain=self.domain, cmap=self.cmap)
