...
def _sign_compare(v1, v2):
    return (v1 > v2) - (v2 > v1)


def _grade_order_compare(grade_order, v1, v2):
...
class GradeProfile(object):
    ...
    def __init__(
        ...
        assert gcounts is None or len(grade_maps) == len(gcounts), (
        ...
        self.compare_function = (
            ...
            else partial(_grade_order_compare, self.grade_order)
        )
    ...
    def write(self):
        ...
        gprof_str = (
            f"{len(self.candidates)};{self.num_voters};{self.grades};{self.grade_order}"
        )
        for g in self.grade_functions:
            g_str = ""
    ...
    def from_string(cls, gprof_str):
        ...
        num_alternatives = int(gprof_data[0])
        num_voters = int(gprof_data[1])
        grades = ast.literal_eval(gprof_data[2])
        grade_order = ast.literal_eval(gprof_data[3])
        grade_by_str = {
            str(g): g for g in grades
        }  # recover str OR numeric grade values
        grade_strs = gprof_data[4:]  # grade maps now start at field[4]
        grade_maps = [
            {
                int(cg.split(":")[0]): grade_by_str[cg.split(":")[1]]
                for cg in gs.split(",")
            }
            if gs != ""
            else {}
        ...
        if len(grade_maps) != num_voters:
            raise Exception("Number of voters does not match the number of grade maps.")
        return cls(
            grade_maps,
            grades,
            candidates=list(range(num_alternatives)),
