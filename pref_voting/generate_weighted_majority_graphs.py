...
def generate_edge_ordered_tournament(num_cands, parity="even"):
    r"""Generate a random uniquely weighted MarginGraph for ``num_cands`` candidates.
    ...
    for i, e_idx in enumerate(edge_indices):
        edges.append(
            (
                _edges[e_idx][0],
                _edges[e_idx][1],
                2 * (i + 1) if parity == "even" else 2 * i + 1,
            )
        )
...
def generate_margin_graph(num_cands, weight_domain=None, parity="even"):
    ...
    assert (
        weight_domain is None
        or isinstance(weight_domain, list)
        and len(weight_domain) > 0
    ), "The weight_domain must be a list with at least one element."
...
def generate_margin_graph_bradley_terry(
    num_cands, num_voters, score_prob_mod=lambda c: np.random.uniform(0, 1)
):
    ...
    for c1, c2 in pairs_of_cands:
        ...
        for n in range(num_voters):
            vote = np.random.choice(
                [1, 0],
                p=[
                    cand_score[c1] / (cand_score[c1] + cand_score[c2]),
                    cand_score[c2] / (cand_score[c1] + cand_score[c2]),
                ],
            )
...
def generate_edge_ordered_tournament_infinite_limit(num_candidates, cov_matrix=None):
    ...
    *An Analysis of Random Elections with Large Numbers of Voters* by Matthew Harrison-Trainor
    ...
    cov_matrix = (
        ...
        else generate_covariance_matrix(num_candidates)
    )
    ...
    def pair(p):
        return (
            ...
            - (num_candidates - p[0]) * (num_candidates - p[0] + 1) // 2
        )
...
def _canonical_tournament(num_verts, edges):
    ...
    for perm in permutations(range(num_verts)):
        bits = tuple(1 if (perm[i], perm[j]) in edge_set else 0 for i, j in pairs)
        if best_bits is None or bits < best_bits:
            best_bits = bits

    canonical_edges = [
        (i, j) if bit else (j, i) for bit, (i, j) in zip(best_bits, pairs)
    ]
...
def enumerate_tournaments(num_cands, candidates=None, cmap=None):
    """
    Enumerate one MajorityGraph representative from each isomorphism class of tournaments on num_cands candidates.
    """
...
def _enumerate_ceots(num_cands, num_edges, partial_ceot, used_nodes, next_node):
    ...
    if len(partial_ceot) < num_edges:
        ...
        for n in available_nodes:
            if (
                n == next_node and next_node < num_cands - 1
            ):  # If n == next_node, we are in Case 3 or Case 4 above
                available_nodes = used_nodes + [next_node + 1]

            for m in available_nodes:
                if (
                    not n == m
                    and (n, m) not in partial_ceot
                    and (m, n) not in partial_ceot
                ):
                    new_ceot = [edge for edge in partial_ceot] + [(n, m)]

                    if not (
                        n == next_node or m == next_node
                    ):  # Then we are in Case 1 above
                        yield from _enumerate_ceots(
                            num_cands, num_edges, new_ceot, used_nodes, next_node
                        )

                    if (
                        n == next_node or m == next_node
                    ) and not m == next_node + 1:  # Then we are in Case 2 or 3 above
...
def _enumerate_ceots_as_edgelist(num_cands):
    ...
    yield from _enumerate_ceots(
        num_cands, num_edges, partial_ceot, used_nodes, next_node
    )
...
def _enumerate_cweots_as_edgelist(num_cands, include_weak_tournaments=True):
    ...
    cweots = (
        dict()
    )  # For isomorphism checking, keep track of the cweots generated so far.
    ...
    for ceot in tqdm(list(_enumerate_ceots_as_edgelist(num_cands))):
        ...
        win_vector = tuple(
            sorted(
                [len([edge for edge in ceot if edge[0] == i]) for i in range(num_cands)]
            )
        )
                                ...
                                (
                                    len(
                                        [
                                            edge
                                            for edge in ceot
                                            if edge[0] == i and edge not in cweot[-1]
                                        ]
                                    ),
                                    len(
                                        [
                                            edge
                                            for edge in ceot
                                            if edge[1] == i and edge not in cweot[-1]
                                        ]
                                    ),
                                )
...
def enumerate_canonical_weakly_edge_ordered_tournaments(
    num_cands, parity="even", include_weak_tournaments=True
):
    """
    ...
    for cweot in _enumerate_cweots_as_edgelist(
        num_cands, include_weak_tournaments=include_weak_tournaments
    ):
        weighted_edges = list()
        ...
        for idx, group in enumerate(reversed(cweot)):
            for e in group:
                weighted_edge = (
                    e[0],
                    e[1],
                    2 * (idx + 1) if parity == "even" else 2 * idx + 1,
                )
