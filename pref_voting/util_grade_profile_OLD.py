...
from pref_voting.mappings import Grade, Utility, _Mapping
...
class UtilGradeProfile(object):
    ...
    def __init__(
        ...
        for g in grade_maps:
            if isinstance(g, dict):
                ...
                _candidates += [x for x in g.graded_candidates if x not in _candidates]
    ...
    def sum(self, c):
        ...
        if not self.has_grade(c):
            return None
        return np.sum([g(c) * n for g, n in zip(*self.grades_counts) if g.has_grade(c)])
