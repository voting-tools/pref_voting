...
def test_strength_matrix(simple_margin_graph, simple_margin_graph2):
    ...
    s_matrix3, cand_to_idx2 = simple_margin_graph2.strength_matrix(
        curr_cands=["a", "c"]
    )
...
def test_description(simple_margin_graph):
    ...
    assert (
        description
        == "MarginGraph([0, 1, 2], [(0, 1, 1), (1, 2, 3), (2, 0, 5)], cmap={0: '0', 1: '1', 2: '2'})"
    )
...
def test_display_with_defeat(simple_margin_graph, capsys):
    ...
    simple_margin_graph.display_with_defeat(
        defeat, cmap={0: "a", 1: "b", 2: "c"}, show_undefeated=False
    )
...
def test_to_latex(simple_margin_graph):
    ...
    mg = MarginGraph(
        [0, 1, 2, 3], [(0, 1, 2), (1, 2, 4), (2, 0, 6), (0, 3, 2), (1, 3, 4), (2, 3, 6)]
    )
...
def test_cycles_with_curr_cands_finds_cycle():
    """Bug 2.1: cycles(curr_cands) built a fresh empty DiGraph and subgraphed *that*
    instead of self.mg, so it always returned [] when curr_cands was given."""
    mg = MajorityGraph([0, 1, 2], [(0, 1), (1, 2), (2, 0)])
    assert mg.cycles() == [[0, 1, 2]]
    # restricting to all candidates must find the same 3-cycle
    assert len(mg.cycles(curr_cands=[0, 1, 2])) == 1
    # restricting to a 2-subset that has no cycle must return []
    assert mg.cycles(curr_cands=[0, 1]) == []


def test_has_cycle_with_curr_cands():
    """Bug 2.1 (has_cycle variant): same empty-graph defect."""
    mg = MajorityGraph([0, 1, 2], [(0, 1), (1, 2), (2, 0)])
    assert mg.has_cycle(curr_cands=[0, 1, 2]) is True
    assert mg.has_cycle(curr_cands=[0, 1]) is False


def test_cycles_curr_cands_inherited_by_margin_and_support_graphs():
    """The same methods are inherited by MarginGraph/SupportGraph."""
    marg = MarginGraph([0, 1, 2], [(0, 1, 1), (1, 2, 3), (2, 0, 5)])
    assert len(marg.cycles(curr_cands=[0, 1, 2])) == 1
    sg = SupportGraph([0, 1, 2], [(0, 1, (4, 3)), (1, 2, (5, 2)), (2, 0, (6, 1))])
    assert len(sg.cycles(curr_cands=[0, 1, 2])) == 1
